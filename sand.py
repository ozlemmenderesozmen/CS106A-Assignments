...
The starter code of the program handles setting up the program window with a user interface, handling the controls and drawing
that allows the user to interact with the program.

With the program, it is easy to draw glass with rocks and put sand into the glass.
Thanks to the parts added by me, by selecting the gravity and brownian options, sand moves down and vibrates.
...
def check_move(grid, x1, y1, x2, y2):
    ...
    always_required_check = (0 <= y2 < y_bound and
           0 <= x2 < x_bound and
           grid[y1][x1] == 's' and
           grid[y2][x2] is None)

    if not always_required_check:
        return False

    if (y1 != y2 and x1 != x2) and grid[y1][x2] is not None:
        return False
